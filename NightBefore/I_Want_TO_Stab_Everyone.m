classdef combinedClass1
    ...
    methods (Static)
        function execution()
            ...
            % Red Muffin
            operation.runRedTrajectories();

            % Green Muffin
            operation.runGreenTrajectories();
    ...
    methods
        ...
        function runRedTrajectories(obj) % Main robot movement loop fucntion
            % NOTE: BOTH Q-MATRICES SHOULD BE THE SAME SIZE TO RUN PROPERLY
            % Create the q-matrix of Dobot waypoints
            obj.q_s_red{1} = obj.s.model.ikcon(transl(-1.600, 0.026, 1.517)*rpy2tr(0,0,pi/2), [-1.5708    0.5236    0.9809    1.6232]); 
            obj.q_s_red{2} = obj.s.model.ikcon(transl(-1.6, -0.009, 1.398)*rpy2tr(0,0,-pi/2), [-1.5708    1.0210    0.9809    1.1432]); 
            obj.q_s_red{3} = obj.q_s_red{1}; % holding first red muffin
            obj.q_s_red{4} = obj.q_s_red{1};
            obj.q_s_red{5} = obj.q_s_red{1};
            obj.q_s_red{6} = obj.q_s_red{1};
            obj.q_s_red{7} = obj.s.model.ikcon(transl(-1.600, 0.552, 1.539)*rpy2tr(0,0,pi/2), [1.5708    0.3534    1.0594    1.7279]);
            obj.q_s_red{8} = obj.s.model.ikcon(transl(-1.600, 0.562, 1.439)*rpy2tr(0,0,pi/2), [1.5709    0.5498    1.4835    1.0996]); 
            obj.q_s_red{9} = obj.s.model.ikcon(transl(-1.600, 0.552, 1.539)*rpy2tr(0,0,pi/2), [1.5708    0.3534    1.0594    1.7279]);
            obj.q_s_red{10} = obj.q_s_red{1};
            obj.q_s_red{11} = obj.q_s_red{1};
            obj.q_s_red{12} = obj.q_s_red{1};
            obj.q_s_red{13} = obj.q_s_red{1};
            obj.q_s_red{14} = obj.q_s_red{1};
            obj.q_s_red{15} = obj.q_s_red{1};

            % Create the q-matrix of Mitsubishi waypoints
            obj.q_r_red{1} = obj.r.model.ikcon(transl(-0.4, 0.566, 1.622)*rpy2tr(-pi/2,-pi/2,-pi/2), [-0.0838    1.1729   -0.3281         0   -0.7540         0]);
            obj.q_r_red{2} = obj.r.model.ikcon(transl(-0.485, 0.6, 1.385)*rpy2tr(-pi/2,-pi/2,-pi/2), [0    0.5027   -0.5515         0   -1.5708         0]); 
            obj.q_r_red{3} = obj.r.model.ikcon(transl(-0.270, 0.6, 1.364)*rpy2tr(pi/2,pi/2,pi/2), [0   -0.1257    0.9006         0   -0.8029         0]);
            obj.q_r_red{4} = obj.r.model.ikcon(transl(-0.219, 0.6, 1.539)*rpy2tr(pi/2,pi/2,pi/2), [0   -0.0314    1.4033         0   -0.2094         0]); 
            obj.q_r_red{5} = obj.r.model.ikcon(transl(-1.379, 0.6, 1.579)*rpy2tr(-pi/2,-pi/2,-pi/2), [ 3.1416   -0.0419    1.5704    0   -0.0419    0]);
            obj.q_r_red{6} = obj.r.model.ikcon(transl(-1.342, 0.6, 1.36)*rpy2tr(-pi/2,-pi/2,-pi/2), [3.1416   -0.4887    1.5708    0   -0.5027         0]); 
            obj.q_r_red{7} = obj.q_r_red{6}; % loading red muffins
            obj.q_r_red{8} = obj.q_r_red{6};
            obj.q_r_red{9} = obj.q_r_red{6};
            obj.q_r_red{10} = obj.q_r_red{6};
            obj.q_r_red{11} = obj.q_r_red{5}; % return red tray to 
            obj.q_r_red{12} = obj.q_r_red{4};
            obj.q_r_red{13} = obj.q_r_red{3};
            obj.q_r_red{14} = obj.q_r_red{2}; % pull out spatula from red tray
            obj.q_r_red{15} = obj.r.model.ikcon(transl(-0.518, 0.6, 1.504)*rpy2tr(-pi/2,-pi/2,-pi/2), [0    1.1310   -0.9146         0   -1.3404         0]); 
            ...
            for i = 1:width(obj.q_s_red)
                % iteration counter
                disp(i);
                if i == 3 || 4 || 5 || 6
                    qMatrixRedMuffin = jtraj(robot_pos,obj.q_s_red{i},steps);
                end

                % Calculate the joint trajectory between q matrix points
                newQ_s = obj.q_s_red{i};
                jointTrajectory_s = jtraj(obj.s.model.getpos(), newQ_s, steps);
                newQ_r = obj.q_r_red{i};
...
%                                         obj.r.model.animate(qMatrixPlace(j,:))   % animate the UR3 to animate through the q Matrix
        ...
        function runGreenTrajectories(obj) % Main robot movement loop fucntion
            % NOTE: BOTH Q-MATRICES SHOULD BE THE SAME SIZE TO RUN PROPERLY
            % Create the q-matrix of Dobot waypoints
            obj.q_s_green{1} = obj.s.model.ikcon(transl(-1.600, 0.026, 1.517)*rpy2tr(0,0,pi/2), [-1.5708    0.5236    0.9809    1.6232]); 
            obj.q_s_green{2} = obj.s.model.ikcon(transl(-1.6, -0.009, 1.398)*rpy2tr(0,0,-pi/2), [-1.5708    1.0210    0.9809    1.1432]); 
            obj.q_s_green{3} = obj.q_s_green{1}; % holding first red muffin
            obj.q_s_green{4} = obj.q_s_green{1};
            obj.q_s_green{5} = obj.q_s_green{1};
            obj.q_s_green{6} = obj.q_s_green{1};
            obj.q_s_green{7} = obj.s.model.ikcon(transl(-1.600, 0.552, 1.539)*rpy2tr(0,0,pi/2), [1.5708    0.3534    1.0594    1.7279]);
            obj.q_s_green{8} = obj.s.model.ikcon(transl(-1.600, 0.562, 1.439)*rpy2tr(0,0,pi/2), [1.5709    0.5498    1.4835    1.0996]); 
            obj.q_s_green{9} = obj.s.model.ikcon(transl(-1.600, 0.552, 1.539)*rpy2tr(0,0,pi/2), [1.5708    0.3534    1.0594    1.7279]);
            obj.q_s_green{10} = obj.q_s_green{1};
            obj.q_s_green{11} = obj.q_s_green{1};
            obj.q_s_green{12} = obj.q_s_green{1};
            obj.q_s_green{13} = obj.q_s_green{1};
            obj.q_s_green{14} = obj.q_s_green{1};
            obj.q_s_green{15} = obj.q_s_green{1};

            % Create the q-matrix of Mitsubishi waypoints
            obj.q_r_green{1} = obj.r.model.ikcon(transl(-0.601, 0.799, 1.504)*rpy2tr(-pi/2,-pi/2,-pi/4), [ 0.7854    1.1248   -0.9130    0   -1.3589   0]);
            obj.q_r_green{2} = obj.r.model.ikcon(transl(-0.577, 0.823, 1.385)*rpy2tr(-pi/2,-pi/2,-pi/4), [0.7854    0.5027   -0.5515         0   -1.5708         0]);
            obj.q_r_green{3} = obj.r.model.ikcon(transl(-0.425, 0.975, 1.364)*rpy2tr(-pi/2,-pi/2,-pi/4), [0.7854   -0.1257    0.9006         0   -0.8029         0]);
            obj.q_r_green{4} = obj.r.model.ikcon(transl(-0.389, 1.011, 1.539)*rpy2tr(-pi/2,-pi/2,-pi/4), [0.7854   -0.0314    1.4033         0   -0.2094         0]);
            obj.q_r_green{5} = obj.r.model.ikcon(transl(-1.379, 0.6, 1.579)*rpy2tr(-pi/2,-pi/2,-pi/2), [ 3.1416   -0.0419    1.5704    0   -0.0419    0]);
            obj.q_r_green{6} = obj.r.model.ikcon(transl(-1.342, 0.6, 1.36)*rpy2tr(-pi/2,-pi/2,-pi/2), [3.1416   -0.4887    1.5708    0   -0.5027         0]); 
            obj.q_r_green{7} = obj.q_r_green{6}; % loading red muffins
            obj.q_r_green{8} = obj.q_r_green{6};
            obj.q_r_green{9} = obj.q_r_green{6};
            obj.q_r_green{10} = obj.q_r_green{6};
            obj.q_r_green{11} = obj.q_r_green{5}; % return red tray to 
            obj.q_r_green{12} = obj.q_r_green{4};
            obj.q_r_green{13} = obj.q_r_green{3};
            obj.q_r_green{14} = obj.q_r_green{2}; % pull out spatula from red tray
            obj.q_r_green{15} = obj.r.model.ikcon(transl(-0.518, 0.6, 1.504)*rpy2tr(-pi/2,-pi/2,-pi/2), [0    1.1310   -0.9146         0   -1.3404         0]); 
...
%             chef = PlaceObject('Chef3.PLY',[1,-1.5,0]);
            ...
            % Main robot movement loop
            for i = 1:width(obj.q_s_green)
                % iteration counter
                disp(i);
                if i == 3 || 4 || 5 || 6
                    qMatrixGreenMuffin = jtraj(robot_pos,obj.q_s_green{i},steps);
                end

                % Calculate the joint trajectory between q matrix points
                newQ_s = obj.q_s_green{i};
                jointTrajectory_s = jtraj(obj.s.model.getpos(), newQ_s, steps);
                newQ_r = obj.q_r_green{i};
                jointTrajectory_r = jtraj(obj.r.model.getpos(), newQ_r, steps);
